...
export interface AppProps {
  connectParams: TransportConnectionParams | ConnectionEndpoint | { webrtcUrl: string };
  transportType: 'daily' | 'webrtc';
}
...
export const App = ({ connectParams, transportType }: AppProps) => {
  ...
  const autoConnect = typeof window !== 'undefined' && (
    process.env.NEXT_PUBLIC_AUTOCONNECT === '1' ||
  ...
  useEffect(() => {
    // Optional: auto-connect for local connectivity checks
    if (!client || !autoConnect) return;
    if (['initialized', 'disconnected', 'error'].includes(client.state)) {
      // Fire and forget
      handleStartSession();
    }
  }, [client, autoConnect]);
  ...
  const handleStartSession = async () => {
    ...
    try {
      // Normalize params for WebRTC
      let params: any = connectParams;
      if (transportType === 'webrtc') {
        const cp = (connectParams || {}) as Record<string, unknown>;
        const url = typeof cp.webrtcUrl === 'string' && cp.webrtcUrl.length > 0
          ? (cp.webrtcUrl as string)
          : 'http://localhost:7860/api/offer';
        params = { webrtcUrl: url };
      }

      console.log('Attempting to connect with:', JSON.stringify({ transportType, params }, null, 2));
      ...
      setError(
        `Failed to start session: ${
          err instanceof Error ? err.message : err ? String(err) : 'Unknown error occurred'
        }`
      );
    }
  };
